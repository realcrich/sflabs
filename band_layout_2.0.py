"""
band_layout.py – v8: Single corridor in courtyard mode + dead-end corridor trimming
...
$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$   CHANGE LOG    $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    ...
    03/08/26 - CR: v8 — two FAR-efficiency improvements:
               1. Courtyard mode now generates a SINGLE corridor (inner ring only).
                  Outer units extend to the inscribed rect boundary, eliminating the
                  redundant outer corr_rect and maximizing floor-plate efficiency.
               2. Dead-end corridor segments adjacent only to residential units are
                  trimmed. Any unit whose inner edge bordered the removed corridor
                  has that edge extended outward by corr_w, recovering the space.
                  Applies to both outer and inner (courtyard) ring units.
...
def _unit_inner_edge(verts):
    """
    Return the index pair (i, j) of the unit's 'inner' edge — the one facing
    the corridor / core interior. In the standard quad layout every unit is
    [o0, o1, i1, i0], so the inner edge is vertices 2-3 (i1→i0), i.e. indices
    (2, 3).  We identify it as the edge whose midpoint is closest to the
    centroid of all four inner vertices.
    """
    # For the canonical winding [o0, o1, i1, i0] the inner edge is always (2,3).
    # Return as (idx_a, idx_b).
    return (2, 3)


def _extend_unit_inner_edge(verts, normal, amount):
    """
    Push the inner edge of a unit polygon outward (toward the core) by `amount`
    feet, recovering corridor space into the unit.

    Args:
        verts  : list of 4 (x,y) tuples — the unit polygon [o0,o1,i1,i0]
        normal : inward-pointing unit normal for this edge (toward the core)
        amount : feet to extend (typically corr_w)

    Returns:
        New list of 4 (x,y) tuples with the inner edge translated by +amount
        along `normal`.
    """
    ia, ib = _unit_inner_edge(verts)
    new_verts = list(verts)
    new_verts[ia] = vadd(verts[ia], vscale(normal, amount))
    new_verts[ib] = vadd(verts[ib], vscale(normal, amount))
    return new_verts
...
def trim_dead_end_corridors(units, corners, inner_units, inner_corners,
    ...
    """
    Identify corridor segments that serve only as dead-end buffers next to
    residential units (no real through-circulation value) and remove them by
    extending adjacent unit polygons to absorb the recovered space.

    Strategy
    --------
    In the standard band layout the outer corridor is a hollow rectangle
    (corr_rect minus core_rect).  Every unit already has its inner edge
    touching corr_rect.  A corridor cell is 'dead-end' when it sits between
    a unit's inner face and the core — i.e. it is *only* bounded by unit
    geometry on both the inner and outer sides without serving as a link
    between distinct circulation zones.

    Concretely: for each of the 4 edges, if ALL units on that edge share the
    same inward normal and there is no BOH/circ/stair attached to that
    corridor segment, we extend every unit on that edge inward by corr_w
    and null out the corr_rect on that side.

    In courtyard mode the outer corridor is already eliminated (Change 1);
    this function still trims any inner corridor stubs around the courtyard
    ring using the same per-edge logic.

    Args:
        units, corners            : outer ring unit lists (polygon, type)
        inner_units, inner_corners: inner ring unit lists (polygon, type)
        corr_rect                 : outer corridor polygon (may be None)
        core_rect                 : service core polygon (may be None)
        courtyard_rect            : open courtyard polygon (may be None)
        edge_normals              : list of 4 inward-pointing unit vectors
        corr_w                    : corridor width in feet
        layout_mode               : 'basic'|'courtyard'|'core_rooms'|'notch'

    Returns:
        trimmed_units        : new outer unit list with extended polygons
        trimmed_inner_units  : new inner unit list with extended polygons
        trimmed_corr_rect    : corr_rect (unchanged; outer corridor was
                               already removed in courtyard mode by Change 1)
    """

    def _extend_list(unit_list, normal):
        """Return a new list with every unit's inner edge pushed by corr_w."""
        result = []
        for verts, utype in unit_list:
            new_verts = _extend_unit_inner_edge(verts, normal, corr_w)
            result.append((new_verts, utype))
        return result

    def _extend_corners(corner_list, normal):
        result = []
        for verts, ctype in corner_list:
            new_verts = _extend_unit_inner_edge(verts, normal, corr_w)
            result.append((new_verts, ctype))
        return result

    # -------------------------------------------------------------------------
    # Outer ring:
    #   - courtyard mode : corr_rect is already None (Change 1). Outer units
    #     must extend inward by corr_w so their inner faces reach the core
    #     boundary, filling the former outer corridor space.
    #   - non-courtyard  : outer corridor still exists. Extend units so their
    #     inner faces absorb dead-end corridor stubs facing the core.
    # In both cases every outer unit is extended by corr_w on its inner face.
    # BOH/circ rooms are placed after this call and their cells overlay units
    # in rasterize(), so extending unconditionally is safe.
    # -------------------------------------------------------------------------
    trimmed_units         = _extend_all_units(units, corr_w)
    trimmed_corners       = _extend_all_corners(corners, corr_w)
    trimmed_inner_units   = list(inner_units)
    trimmed_inner_corners = list(inner_corners)

    # Inner ring (courtyard mode): extend inner units inward by corr_w so their
    # inner faces reach the courtyard boundary, absorbing the inner corridor stub.
    if layout_mode == 'courtyard':
        trimmed_inner_units   = _extend_all_units(inner_units, corr_w)
        trimmed_inner_corners = _extend_all_corners(inner_corners, corr_w)

    return trimmed_units, trimmed_corners, trimmed_inner_units, trimmed_inner_corners, corr_rect


def _extend_all_units(unit_list, amount):
    """
    Extend every unit's inner edge (indices 2,3) outward by `amount` feet,
    using the per-unit computed inward normal.

    The inward normal for each unit is derived from the vector from the
    outer-edge midpoint to the inner-edge midpoint.
    """
    result = []
    for verts, utype in unit_list:
...
def _extend_all_corners(corner_list, amount):
    result = []
    for verts, ctype in corner_list:
        normal = _unit_inward_normal(verts)
        new_verts = _extend_unit_inner_edge(verts, normal, amount)
